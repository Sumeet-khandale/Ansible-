...
* Reformat the consolidated steps as a ready-to-copy `README.md` section.
* Produce a shorter TL;DR comparison table or a GitHub-flavored README file ready to commit.
* Add exact timestamps or command outputs inline.

Tell me which of the above you want and I'll update the canvas accordingly.

## 14. Created Ansible role — kubernetes (on new EC2)

Logged into another EC2 host (controller with Ansible already installed) having the files `first-playbook.yml` and `inventary`.

### Commands executed

```bash
ls
# output: first-playbook.yml  inventary

ansible-galaxy role init kubernetes
# → Role kubernetes was created successfully

ls
# output: first-playbook.yml  inventary  kubernetes

cd kubernetes
ls
# output: README.md  defaults  files  handlers  meta  tasks  templates  tests  vars
```

Attempted `ls -ltr kubernetes` inside the same folder (returned “No such file or directory” because already inside the `kubernetes` directory).

### Final structure after role creation
...
Each of these directories serves a standard purpose in an Ansible role:

* **defaults/** → Default variables for the role.
* **files/** → Static files to be copied to the target.
* **handlers/** → Handlers triggered by tasks (e.g., restart services).
* **meta/** → Role dependencies and metadata.
* **tasks/** → Main list of tasks to run.
* **templates/** → Jinja2 templates for configuration files.
* **tests/** → Sample test inventory and playbook.
* **vars/** → Variables with higher priority than defaults.

The folder was created successfully and ready for customization (e.g., adding Kubernetes installation tasks inside `tasks/main.yml`).
